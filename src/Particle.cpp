...
namespace md {
    ...
    /// -----------------------------------------
    Particle::Particle(const int type) : position({}), velocity({}), force({}), old_force({}), mass(0), type(type) {
    ...
    /// -----------------------------------------
    ParticleContainer::ParticleContainer(const std::vector<Particle>& particles) : particles(particles) {}

    ParticleContainer::ParticleContainer(std::vector<Particle>&& particles) : particles(std::move(particles)) {}

    void ParticleContainer::add_cuboid(const vec3& origin, const vec3& init_v,
                                       const std::array<uint32_t, 3>& num_particles, double thermal_v, double width,
                                       double mass, uint8_t dimension, int type) {
    ...
    Particle& ParticleContainer::operator[](const size_t index) { return particles[index]; }

    const Particle& ParticleContainer::operator[](const size_t index) const { return particles[index]; }
