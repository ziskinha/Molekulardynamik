...
namespace md::utils {
    ...
    class PairIterator final {
       public:
        ...
        bool operator==(const PairIterator& other) const { return inner == other.inner && outer == other.outer; }

        bool operator!=(const PairIterator& other) const { return !(*this == other); }

        reference operator*() const { return {*outer, *inner}; }

        reference operator*() { return {*outer, *inner}; }
       ...
       private:
        PairIterator(Container& inner_container, Container& outer_container, ResetFunction increment_outer)
            : inner(inner_container.begin()),
              outer(outer_container.begin()),
              inner_container(inner_container),
              outer_container(outer_container),
              increment_outer(increment_outer) {}
    ...
    class DualPairIterator final {
       public:
        ...
        PairIterator<Container> end() { return PairIterator<Container>::end(inner_container, outer_container); }
